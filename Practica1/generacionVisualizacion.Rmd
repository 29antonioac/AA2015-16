...
```{r plotMediaSimetria}
datos_1 <- data.frame(Medias = as.numeric(medias_1), Simetrias = as.numeric(simetrias_1), Etiqueta = 1)
datos_5 <- data.frame(Medias = as.numeric(medias_5), Simetrias = as.numeric(simetrias_5), Etiqueta = -1)
...
Aplicamos el algoritmo de regresión para obtener un clasificador lineal.
...
regres_lin <- function(datos, label, pesos = FALSE) {
  ...
  if (is.null(ncol(datos)) || pesos == TRUE)
    return(w)
  else
...
Una vez tenemos el algoritmo, lo probamos y dibujamos la recta.

```{r pruebaRegresion}
# muestras <- cbind(datos$Medias, datos$Simetrias)
# etiquetas <- datos$Etiqueta
muestras <- datos$Medias
etiquetas <- datos$Simetrias
recta <- regres_lin(muestras, etiquetas)
plot(datos$Medias, datos$Simetrias, main = "Intensidad y Simetría", xlab = "Intensidad", ylab = "Simetría Vertical", col = as.numeric(datos$Etiqueta) + 2)
abline(recta)
```
...
El $E_{in}$ es `r I(errores)`, lo cual nos da una estimación de lo bueno que puede ser nuestro clasificador dentro de la muestra.
...
El $E_{out}$ es `r I(errores)`, lo cual nos da a entender que fuera de la muestra, también se comporta bien nuestro clasificador.
...
muestra <- simula_unif(1000, 2, c(-10,10))

etiquetas <- apply(muestra,1, function(m) { do.call(f, as.list(m)) } )

porcion <- 10
numero.datos.ruido <- ceiling(nrow(muestra) * porcion / 100)


ruido <- sample(c(rep(-1,numero.datos.ruido), 
                                         rep(1, nrow(muestra) - numero.datos.ruido)))
...
par(mfrow=c(1,2))
plot(muestra, main = "Muestras originales", col = ifelse(etiquetas == 1, "red", "blue"))
plot(muestra, main = "Muestras con ruido", col = ifelse(etiquetas.ruido == 1, "red", "blue"))
```

Ahora ajustaremos una regresión y veremos el error dentro de esta muestra.
